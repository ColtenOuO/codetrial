  ...
  try {
    ...
    const cases = runnable.cases.map((testCase, index) => {
      ...
      if (!result || result.error) {
        return { label: testCase.label, pass: false, got: "-", expected: observed ? undefined : renderValue(testCase.expected), error: result?.error || "No result produced.", timeMs: Math.round(result?.timeMs || 0), candidate };
      }
      let pass = null;
      let error = null;
      if (!observed) {

...
fetch_manifest()
{
    ...
    while read -r line || [ -n "$line" ]; do
        ...
        # One filename per line, serving as both the URL suffix and the path
        # under the vendor directory, so it takes only what a vendored filename
        # can be. This rejects a stray carriage return from a CRLF checkout,
        # which would otherwise 404 against a URL that looks correct in the
        # error message; it rejects a `/` before one can write outside
        # `web/vendor/`; and it rejects a second field, since the space comes
        # through in the match.
        #
        # There used to be a two-field form here, for the one asset whose
        # upstream name differed from the one this tree served. That was the
        # avatar model, which the browser now fetches for itself, and no
        # manifest has used a second field since.
        name=$line
        case "$name" in
            *[!A-Za-z0-9._-]* | .*)
        ...
        # Per-process, because `make -j` can run the fetch-vendor prerequisite
        # of build, serve, and web concurrently and a fixed name is a race.
        tmp="$dir/.$name.$$.part"
        ...
        if ! mv "$tmp" "$dir/$name"; then
            ...
            # in place, dropped a `.part` file that then fails verify-vendor
            # forever, and still exited 0.
            rm -f "$tmp"

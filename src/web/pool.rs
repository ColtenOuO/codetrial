...
impl ProviderVerdict {
    ...
    /// The API code a caller reports when this verdict is why an interview
    /// cannot start. Here rather than at the consumer: a new variant that
    /// forgets this match fails to compile, where an `any(matches!(..))` at
    /// the caller would have gone on reporting a spent quota for it.
    pub(crate) fn code(self) -> &'static str {
        match self {
            Self::Available | Self::OutOfMinutes => "livekit_quota_exhausted",
            Self::CredentialRefused(_) => "livekit_provider_credential_refused",
        }
    }

    /// Which verdict speaks for the pool when several projects are down at
    /// once. A refused credential is an account to fix and a spent quota is
    /// minutes to buy, so the refusal is the more actionable thing to say.
    fn severity(self) -> u8 {
        match self {
            Self::Available => 0,
            Self::OutOfMinutes => 1,
            Self::CredentialRefused(_) => 2,
        }
    }

    /// The verdict a caller should report for a whole pool.
    pub(crate) fn worst(verdicts: impl IntoIterator<Item = ProviderVerdict>) -> ProviderVerdict {
        verdicts
            .into_iter()
            .max_by_key(|verdict| verdict.severity())
            .unwrap_or(Self::OutOfMinutes)
    }
...
impl ProviderQuota {
    ...
    pub(crate) async fn verdict_for(&self, provider: &Provider) -> ProviderVerdict {
        self.verdict_for_at(provider, Instant::now()).await
    }

    async fn verdict_for_at(&self, provider: &Provider, now: Instant) -> ProviderVerdict {
        if !self.probe {
...
async fn probe_verdict(provider: &Provider) -> ProviderVerdict {
    ...
    match response.map(|response| response.status()) {
        Ok(StatusCode::TOO_MANY_REQUESTS) => ProviderVerdict::OutOfMinutes,
        Ok(status @ (StatusCode::UNAUTHORIZED | StatusCode::FORBIDDEN)) => {
            ProviderVerdict::CredentialRefused(status)
        }
